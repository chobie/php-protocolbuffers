--TEST--
Check for protocol buffers unknown field double implementations
--FILE--
<?php

class TestDouble extends ProtocolBuffersMessage
{
    protected $value;
...
foreach ($field->getAsDoubleList() as $val) {
    if (round($val, 2) == 3.14) {
        echo "OK" . PHP_EOL;
    } else {
        echo "Failed: " . var_export($val);
